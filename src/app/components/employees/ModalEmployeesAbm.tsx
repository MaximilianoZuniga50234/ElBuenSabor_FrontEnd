...
export default function ModalEmployeesAbm({
  employee,
  open,
  setOpen,
  isNew,
  ...
  const [isConnectionDeleted, setIsConnectionDeleted] =
    useState<boolean>(false);
  const [isConfirmButtonPressed, setIsConfirmButtonPressed] =
    useState<boolean>(false);
  const [employeePost, setEmployeePost] = useState<UserAuth0Post>(
    employeePostInitialState
  );
  ...
  useEffect(() => {
    setEmployeePost({
      ...
      user_metadata: {
        ...
        roleToAdd: employee.role != "" ? employee.role : "Cajero",
      },
  ...
  const handleChangeName = (event: React.ChangeEvent<HTMLInputElement>) => {
    setEmployeePost({
      ...
      name: `${event.target.value} ${employeePost.family_name}`,
    });
  };
  ...
  const handleChangeFamilyName = (
    ...
    setEmployeePost({
      ...
      name: `${employeePost.given_name} ${event.target.value}`,
    });
  };
  ...
  const handleChangeAddressNumber = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setEmployeePost({
      ...employeePost,
  ...
  const handleClickPhoneNumberPopover = (
  ...
  const [emailAnchorEl, setEmailAnchorEl] = useState<HTMLButtonElement | null>(
    null
  );

  const openEmailPopover = Boolean(emailAnchorEl);

  const handleClickEmailPopover = (
    event: React.MouseEvent<HTMLButtonElement>
  ) => {
  ...
  const handleClickPasswordPopover = (
    event: React.MouseEvent<HTMLButtonElement>
  ) => {
