...
export default function ModalCustomersAbm({
  customer,
  open,
  setOpen,
}: ModalCustomersProps) {
  ...
  const [isConfirmButtonPressed, setIsConfirmButtonPressed] =
    useState<boolean>(false);
  const [customerPost, setCustomerPost] = useState<UserAuth0Post>(
    customerPostInitialState
  );
    ...
    setCustomerPost({
      ...
      user_metadata: {
        ...
        address: {
          ...
          department:
            customer.user_metadata.address.department != ""
              ? customer.user_metadata.address.department
              : "Ciudad",
        },
      },
  ...
  const handleChangeName = (event: React.ChangeEvent<HTMLInputElement>) => {
    setCustomerPost({
      ...
      name: `${event.target.value} ${customerPost.family_name}`,
    });
  };
  ...
  const handleChangeFamilyName = (
    ...
    setCustomerPost({
      ...
      name: `${customerPost.given_name} ${event.target.value}`,
    });
  };
