...
export default function ModalStockPurchase({ stock, setStock, isOpen, handleClose }: ModalStockPurchaseProps) {
    ...
    useEffect(() => {
        if (stockPurchased) {
            setStockModified({ ...stockModified, price: stockPurchased.purchasePrice })
        }

        if (stockPurchased.purchasePrice != 0 && stockPurchased.purchasePrice === stockModified.price) {
    ...
    const handleChangeStock = (event: React.ChangeEvent<HTMLSelectElement>) => {
        const stockFind = stocks.find((stock) => stock.denomination === event.target.value)
        if (stockFind) {
    ...
    const handleChangeCurrentStock = (event: React.ChangeEvent<HTMLInputElement>) => {
        if (stockPurchased) {
            setStockModified({ ...stockModified, quantityPurchased: Number(event.target.value) })
        }
    }

    const handleConfirm = async () => {
        setStockPurchased({ ...stockPurchased, purchasePrice: stockModified.price, currentStock: stockPurchased.currentStock + stockModified.quantityPurchased })
    ...
    const handleCloseModal = () => {
        handleClose()
        setStockModified({ price: stockPurchased.purchasePrice, quantityPurchased: 0 })
    }
    ...
    return (
        <>
                ...
                <Fade in={isOpen}>
                    <Box className='modalStockPurchase__box'>
                        ...
                        <div className="modalStockPurchase__itemStock">
                            ...
                            >
                                {stocks.map((stock: Stock) => (
                                    <option className="modalStockPurchase__option" key={stock.id} disabled={stock.active === true ? false : true}>{stock.denomination}</option>
                                ))}
                            </select>
                        </div>

                        <button className="modalStockPurchase__button modalStockPurchase__button--addStock" onClick={handleOpenModalStock}>
                        ...
                        <div className="modalStockPurchase__quantityPurchased">
                            <label htmlFor="modalStockPurchase__input">Cantidad comprada</label>
                            <input type="number"
                                className="modalStockPurchase__input"
                                defaultValue="0"
                                min={0}
                        ...
                        <div className="modalStockPurchase__buttons">
                            <button className="modalStockPurchase__button" onClick={handleCloseModal}>Cancelar</button>
                            <button className="modalStockPurchase__button" onClick={
                                handleConfirm
                            }
                            >Confirmar</button>
                        </div>
            ...
            <ModalStock
                stock={stock} setStock={setStock} isOpen={openModalStock} handleClose={handleCloseModalStock} isNew={true}
            ></ModalStock>
        </>
    )
}
