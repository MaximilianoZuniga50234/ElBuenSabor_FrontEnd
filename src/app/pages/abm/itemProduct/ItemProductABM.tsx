...
import { useStore as useItemProducts } from "../../../store/ItemProductStore"
import { FaPencilAlt } from "react-icons/fa"
import { useEffect, useState } from "react";
import { Modal, Fade, Box } from "@mui/material";
import { ItemProduct } from "../../../interfaces/ItemProduct";
import { addItemProduct, updateItemProduct } from "../../../functions/ItemProductAPI";
import { useAuth0 } from "@auth0/auth0-react";
import { toast, Toaster } from "sonner";

export default function ItemProductABM() {
    const { itemProducts } = useItemProducts()
    const [itemProduct, setItemProduct] = useState<ItemProduct | null>({ id: 0, denomination: "", active: true })
            ...
            <div className="itemProductABM__table">
                <div className="itemProductABM__header">
                    ...
                    <div className="itemProductABM__button__container">
                        <button className="itemProductABM__button" onClick={() => handleAdd()}>
                            Añadir nuevo rubro
                        </button>
                    </div>
                </div>
                ...
                <div className="itemProductABM__rows__container">
                    {itemProducts.map(itemProduct =>
                        <div className="itemProductABM__row" key={itemProduct.id}>
                            ...
                            <h4 className="itemProductABM__h4">{itemProduct.denomination}</h4>
                            <h4 className="itemProductABM__h4">{itemProduct.active === true ? 'De alta' : 'De baja'}</h4>
                            <div className="itemProductABM__icon">
                                <button className="itemProductABM__button itemProductABM__button--icon" onClick={() => handleModify(itemProduct)}>
                                    <FaPencilAlt />
                                </button>
                            </div>
                        </div>
                ...
                <Fade in={open}>
                    <Box className='itemProductABM__modal__box'>
                        ...
                        <div className="itemProductABM__modal__name">
                            <label htmlFor="itemProductABM__modal__input">Nombre del rubro</label>
                            <input type="text"
                        ...
                        <div className="itemProductABM__modal__state">
                            ...
                            <label htmlFor="itemProductABM__modal__select">Estado del rubro</label>
                            <select className="itemProductABM__modal__select" defaultValue={itemProduct?.active === true ? 'De alta' : 'De baja'} onChange={handleChangeOption}>
                                <option className="itemProductABM__modal__option">De alta</option>
                                <option className="itemProductABM__modal__option">De baja</option>
                            </select>
                        </div>

                        <div className="itemProductABM__modal__buttons">
                            <button className="itemProductABM__modal__button" onClick={handleClose}>Cancelar</button>
                            <button className="itemProductABM__modal__button" onClick={function () {
