...
export default function LowStock() {
    ...
    const getStocks = async () => {
        ...
        const filterStocks = stocksResponse.filter((stock: Stock) => stock.minimumStock > stock.currentStock
                ...
                <div className="lowStockPage__rows__container">
                    {stocks.map((stock: Stock) =>
                        <div className="lowStockPage__row" key={stock.id}>
                            ...
                            <h4 className="lowStockPage__h4">{stock.measurementUnit.name}</h4>
                            <h4 className="lowStockPage__h4">{stock.minimumStock}</h4>
                            <h4 className="lowStockPage__h4">{stock.currentStock}</h4>
                            <h4 className="lowStockPage__h4">{Math.abs(stock.currentStock - stock.minimumStock)}</h4>
                            <div className="lowStockPage__icon">
                                <button className="lowStockPage__button lowStockPage__button--icon" onClick={() => { handleRegisterPurchase(stock) }}>
                                    <FaPlus />
                                </button>
                            </div>
                        </div>
